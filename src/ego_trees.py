...
def calculate_all_locally_balanced_egotrees(demand_distribution, delta, indexes: List, prefix, helper_struct):
    egotrees = calculate_all_egotrees(delta, demand_distribution, indexes, prefix)

    egotrees, new_demand_matrix = change_nodes_in_egotrees(demand_distribution, delta, egotrees, prefix, helper_struct, local_balance)
    return egotrees, new_demand_matrix

def calculate_all_ego_balanced_egotrees(demand_distribution, delta, indexes: List, prefix, helper_struct):
    egotrees = calculate_all_egotrees(delta, demand_distribution, indexes, prefix)

    egotrees, new_demand_matrix = change_nodes_in_egotrees(demand_distribution, delta, egotrees, prefix, helper_struct, ego_balance)
    return egotrees, new_demand_matrix

def calculate_all_egotrees(delta, demand_distribution, indexes, prefix):
    egotrees = []
...
def change_nodes_in_egotrees(demand_matrix, delta, egotrees, prefix, helper_struct, balance_strategy: Callable):
    ...
    for tree in egotrees:
        ...
        for struct in helper_struct:
            ...
            if not struct[2].index in leaf_indices:
                ...
                balance_strategy(l_index, l_parent, l_tree, new_demand_matrix, tree, u_index, v_index, v_parent,
                              v_tree)
...
def local_balance(l_index, l_parent, l_tree, new_demand_matrix, tree, u_index, v_index, v_parent, v_tree):
...
def ego_balance(l_index, l_parent, l_tree, new_demand_matrix, tree, u_index, v_index, v_parent, v_tree):
    if new_demand_matrix[u_index][l_index] > new_demand_matrix[u_index][v_index] \
            or tree.get_node_dept(v_tree.root) > tree.get_node_dept(l_tree.root):
        # Toroljuk V-t a fabol
